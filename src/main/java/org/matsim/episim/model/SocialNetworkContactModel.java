 ...
 */
public final class SocialNetworkContactModel extends AbstractContactModel {

	private static final Logger log = LogManager.getLogger(SocialNetworkContactModel.class);
	 ...
	 */
	List<EpisimPerson> otherPersonsInContainerInSN = new ArrayList<>();
	List<EpisimPerson> otherPersonsInContainerNotInSN = new ArrayList<>();
	...
	@Inject
	/* package */
	SocialNetworkContactModel(SplittableRandom rnd, Config config,
	...
	private void infectionDynamicsGeneralized(EpisimPerson personLeavingContainer, EpisimContainer<?> container, double now) {
		...
		for(EpisimPerson person : container.getPersons()){
			if(personLeavingContainer.getSiloPerson().getAlterLists().contains(person.getSiloPerson().getId())){
				otherPersonsInContainerInSN.add(person);
			}else {
				otherPersonsInContainerNotInSN.add(person);
			}
		}

		otherPersonsInContainerNotInSN.remove(personLeavingContainer);
...
//		int contactWith = Math.min(otherPersonsInContainer.size(), Math.max((int) (episimConfig.getSampleSize() * 10), 3));
		int contactWithInSN = Math.min(otherPersonsInContainerInSN.size(), (int)episimConfig.getMaxContacts());

		for (int ii = 0; ii < contactWithInSN; ii++) {
			...
			EpisimPerson contactPerson = otherPersonsInContainerInSN.remove(rnd.nextInt(otherPersonsInContainerInSN.size()));
			...
			if (container instanceof InfectionEventHandler.EpisimFacility) {
				...
				if (infectionType.indexOf("home") >= 0 && infectionType.indexOf("leis") == -1 && infectionType.indexOf("work") == -1
					...
					continue;
				}else if (infectionType.indexOf("nursing") >= 0 && !(leavingPersonsActivity.startsWith("nursing"))) {
					//nursing home/work can only interact with nursing work/home
					continue;
				}
			...
			//contact intensity increase by 1.5 for people in the same social network
			double contactIntensity = Math.min(leavingParams.getContactIntensity(), contactParams.getContactIntensity())*10;
		...
		if(contactWithInSN<(int)episimConfig.getMaxContacts()){
			int contactWithNotInSN = Math.min(otherPersonsInContainerNotInSN.size(),(int)episimConfig.getMaxContacts()-otherPersonsInContainerInSN.size());
			for (int ii = 0; ii < contactWithNotInSN; ii++) {
				...
				// Draw the contact person and remove it -> we don't want to draw it multiple times
				EpisimPerson contactPerson = otherPersonsInContainerNotInSN.remove(rnd.nextInt(otherPersonsInContainerNotInSN.size()));
		...
		otherPersonsInContainerInSN.clear();
		otherPersonsInContainerNotInSN.clear();
	}
