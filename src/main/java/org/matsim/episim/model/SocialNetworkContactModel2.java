 ...
 */
public final class SocialNetworkContactModel2 extends AbstractContactModel {

	private static final Logger log = LogManager.getLogger(SocialNetworkContactModel2.class);
	...
	@Inject
	/* package */
    SocialNetworkContactModel2(SplittableRandom rnd, Config config,
...
//		int contactWith = Math.min(otherPersonsInContainer.size(), Math.max((int) (episimConfig.getSampleSize() * 10), 3));
		...
		for (EpisimPerson p : otherPersonsInContainerInSN) {
		...
		if(otherPersonsInContainerInSN.size()<(int)episimConfig.getMaxContacts()){
